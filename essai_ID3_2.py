...
class Arbre_ID3:
   ...
   def __afficher_arbre(self, noeud, nb_tabs=0):
       ...
       if isinstance(noeud, Noeud):
           ...
           rep=input()
           T.append([nb_tabs,noeud.attribut_teste])
           L.append('\t' * nb_tabs + noeud.attribut_teste)
...
def exemple_utilisation():
   ...
   '''
   exemple = Exemple(["vivant", "sportif", "politique", "scientifique","femme"],
                     ["oui", "non"])
...
def afficher2(self):
       """
           affiche l'entièreté de l'arbre à l'écran
       """
       self.__afficher_arbre2(self.arbre)

def __afficher_arbre2(self, noeud, nb_tabs=0):
       """
           selon la convention :
               <texte> <-> nom de l'attribut
               -<texte> <-> valeur de l'attribut
...
arbre1 = Arbre_ID3('datas PlayTennis.txt')
arbre1.construire()
arbre1.afficher2()
'''
...
for k in range(1,len(T)):
    if T[k-1]=='oui' or T[k-1]=='non':
        if len(T[k])==2:
            T[k].append(T[k-1])

for k in range(len(T)):
    if T[k]!= 'oui' and T[k]!= 'non':
        F.append(T[k])
...
for k in range(len(F)):
    nombre.append(F[k][0])
n=max(nombre)#donne le nombre maximum de questions
